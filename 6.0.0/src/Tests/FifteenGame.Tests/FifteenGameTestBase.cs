...
namespace FifteenGame.Tests
{
    public abstract class FifteenGameTestBase : AbpIntegratedTestBase<FifteenGameTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected FifteenGameTestBase()
        {
        ...
        protected void UsingDbContext(Action<FifteenGameDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<FifteenGameDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<FifteenGameDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<FifteenGameDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<FifteenGameDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<FifteenGameDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<FifteenGameDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<FifteenGameDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<FifteenGameDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<FifteenGameDbContext, Task<T>> func)
        {
            T result;
